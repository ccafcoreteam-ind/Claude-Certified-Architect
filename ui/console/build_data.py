...
def render(domains, themes, tasks, scenarios, questions, cheat):
    P = []
    P.append("/* CCA Teaching Console — content model.")
    ...
    P.append("")
    P.append("  window.CCA = { domains, themes, tasks, scenarios, questions, cheat,")
...
def main():
    ...
    out = render(DOMAINS, THEMES, tasks, SCENARIOS, questions, cheat)
    dest = os.path.join(HERE, "assets", "data.js")
    open(dest, "w", encoding="utf-8").write(out)
    print(f"wrote {os.path.relpath(dest, REPO)}  ({len(out)//1024} KB)")
    print(f"tasks: {len(tasks)} · questions: {len(questions)} · cheat: {len(cheat)} · scenarios: {len(SCENARIOS)}")
